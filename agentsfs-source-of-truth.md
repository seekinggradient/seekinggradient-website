...
It is files + conventions + tools + instructions, and nothing else. An agentsfs instance is a plain git repo of markdown files following a few simple conventions (frontmatter descriptions, wikilinks, a self-describing root). Around it sits a thin toolkit (CLI + MCP) providing search, navigation, and health checks, plus a pack of prompts that teach any agent — Claude Code, Codex, OpenClaw, anything — how to read, write, and maintain the substrate. The intelligence lives in the user's agents; agentsfs makes their knowledge survive and compound.
