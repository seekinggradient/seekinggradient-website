...
- The first-class `/books` area and its long-form web readers are documented in [[Books Library]].

Recent state:

- The [[Books Library]] publishes the four-book Foundation Models series with online readers, EPUB and Markdown downloads, optimized illustrations, Math rendering, chapter navigation, and a dedicated homepage feature.
...
- The homepage keeps a simple first-person information architecture but now uses a bright yellow poster hero, a code-native gradient path, highlighted words, a cobalt interest ribbon, a cobalt [[Books Library]] shelf, colorful project workbench tiles, and a dark personal About section. The visual idea is “warm paper plus saturated marker ink.”
...
- Main navigation is Writing, Books, Projects, About, and Email. The Projects area adds a small local Repository, Mockups, and About subnav. Tracking remains accessible from the footer.
...
- Responsive browser verification covers the homepage, [[Books Library]], book detail and reader pages, ideas index, essay layout, tracking page, and mobile navigation from 320px through 1440px. `npx astro build` is the safe verification command because it avoids the network-sensitive TTS prebuild workflow.
