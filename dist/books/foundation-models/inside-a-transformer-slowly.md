...
![The tensor shape changes from token IDs to embeddings, remains stable through the Transformer blocks, and expands to one logit per vocabulary entry](../../media/transformer-foundations/11-tensor-shape-journey.png)
...
![Karpathy shows Hugging Face’s FineWeb article and its URL filtering, text extraction, language filtering, quality filtering, deduplication, and PII-processing pipeline](../../media/transformer-foundations/06-fineweb-pipeline-at-01m27s.png)
...
![An illustrated refinery turns messy web pages into cleaned documents, subword tiles, and packed training batches](../../media/transformer-foundations/09-data-refinery-illustration.png)
...
![The FineWeb dataset viewer shows rows with cleaned text and provenance fields such as an ID, Common Crawl dump, and source URL](../../media/transformer-foundations/07-fineweb-examples-at-06m16s.png)
...
![Karpathy concatenates cleaned web documents into a huge tapestry of raw text before tokenization](../../media/transformer-foundations/08-training-text-at-07m22s.png)
...
![Cleaned documents are split, marked with end-of-document tokens, packed into fixed-length rows, and shifted to make inputs and targets](../../media/transformer-foundations/13-document-packing.png)
...
![Karpathy demonstrates that “hello world” becomes two token IDs in Tiktokenizer](../../media/transformer-foundations/01-tokenization-at-12m55s.png)
...
![A four-token context enters a neural network and produces probabilities for all 100,277 possible next tokens](../../media/transformer-foundations/02-network-io-at-17m30s.png)
...
![A causal attention map has one query row per token, one key column per token, a masked future, and a weighted read over permitted positions](../../media/transformer-foundations/12-causal-attention-map.png)
...
![A continuous residual ribbon receives information gathered across token positions on the left, then passes through an expanded per-token feature workshop on the right](../../media/transformer-foundations/10-transformer-block-workshop-illustration.png)
...
![Karpathy shows the full nano-GPT stack in Brendan Bycroft’s LLM visualizer](../../media/transformer-foundations/03-transformer-visualizer-at-23m40s.png)
...
![Karpathy’s terminal shows GPT-2 training step, loss, gradient norm, learning rate, utilization, throughput, and generated samples](../../media/transformer-foundations/05-gpt2-training-at-35m20s.png)
...
![Karpathy’s diagram shows sampled tokens being appended and fed back into the model](../../media/transformer-foundations/04-autoregressive-inference-at-27m55s.png)
